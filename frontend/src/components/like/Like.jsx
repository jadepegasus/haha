...
const Like = ({ post_id, user_id, update, poster, react, setReact }) => {
  ...
  return (
    <div
      ...
      onMouseEnter={(e) => {
        reactIcon.current?.classList?.remove("hidden");
      }}
      onMouseLeave={(e) => {
    ...
    >
      {react === "none" && (
        ...
        >
          <span className="group-hover:animate-shaking-like 3xl:text-[30px] text-[calc(30px/6*5)] me-1">
      ...
      <div
        className="bg-white 3xl:h-[42px] h-[calc(42px/6*5)] border-blue-500 border-[1px] rounded-[10px] 3xl:w-[238px] w-[calc(238px/6*5)] px-3 z-10 absolute -translate-y-full justify-center animate-slip-to-top hidden"
        ref={reactIcon}
      >
